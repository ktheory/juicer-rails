...
In your views, use the `juiced_tag` in place of `javascript_include_tag` and
`stylesheet_link_tag`.
...
`javascript/app.js`, and write a script tag for each one. E.g., say app.js
...
the asset is determined by putting `min` before the file extension.
